// https://github.com/AvaloniaUI/Avalonia.Labs/tree/main/src/Avalonia.Labs.Panels/FlexPanel.cs
...
{
    private readonly static Func<Layoutable, int> OrderGetter = x => x is { } y ? Flex.GetOrder(y) : 0;
    private readonly static Func<Layoutable, bool> VisibleGetter = x => x.IsVisible;

    /// <summary>
    ///     Defines the <see cref="Direction" /> property.
    /// </summary>
    public readonly static StyledProperty<FlexDirection> DirectionProperty =
        AvaloniaProperty.Register<FlexPanel, FlexDirection>(nameof(Direction));

    /// <summary>
    ///     Defines the <see cref="JustifyContent" /> property.
    /// </summary>
    public readonly static StyledProperty<JustifyContent> JustifyContentProperty =
        AvaloniaProperty.Register<FlexPanel, JustifyContent>(nameof(JustifyContent));

    /// <summary>
    ///     Defines the <see cref="AlignItems" /> property.
    /// </summary>
    public readonly static StyledProperty<AlignItems> AlignItemsProperty =
    ...
    /// </summary>
    public readonly static StyledProperty<AlignContent> AlignContentProperty =
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        children = children.Where(VisibleGetter).OrderBy(OrderGetter).ToArray();
    ...
    private static (double v, double spacingV) GetCrossAxisPosAndSpacing(
    ...
    private static (double u, double spacingU) GetMainAxisPosAndSpacing(
        JustifyContent justifyContent,
        FlexLine line,
        Uv spacing,
        double remainingFreeU,
        int itemsCount)
    {
        return line.Grow > 0 ?
            (0.0, spacing.U) :
            justifyContent switch
            {
    ...
    private static (double LineMult, double LineAutoMargins, double RemainingFreeU) GetLineMultInfo(FlexLine line, double freeU)
    {
        ...
        return lineMult is > 0 and < 1 ? (lineMult, line.AutoMargins, freeU * lineMult) : (lineMult, line.AutoMargins, freeU);
    }
    ...
    private class FlexLine(int first, int last, LineData l)
    {
